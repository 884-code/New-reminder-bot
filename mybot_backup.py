    ...
    try:
        ...
        if not ch:
            try:
                ...
                ch=await guild.create_text_channel(f"to-{assignee.display_name}", overwrites=ow, topic=f"{assignee.display_name}の個人タスク管理チャンネル")
                intro=_dd.Embed(title="📋 個人タスクチャンネル",
                                description=f"こんにちは、{assignee.display_name}さん！\nこのチャンネルでタスクの通知を受け取ります。",
        ...
        th=await main.create_thread(name=f"🟥 {task_name} - 詳細", auto_archive_duration=60, reason="タスク詳細情報")
        det=_dd.Embed(title="📋 タスク詳細", color=_dd.Color.blue())
        det.add_field(name="指示者", value=instructor.mention, inline=True)
        det.add_field(name="状態", value="🟥 未受託", inline=True)
        det.add_field(name="作成日時", value=_dt.datetime.now().strftime("%Y/%m/%d %H:%M"), inline=True)
...
def _emoji(st): return {"pending":"🟥","accepted":"🟨","completed":"🟩","abandoned":"⚠️","declined":"❌"}.get(st,"⚪")
...
async def handle_task_action(interaction, action, task_id, assignee_id, instructor_id, current_status):
    ...
    try:
        if interaction.message.embeds:
            title=(interaction.message.embeds[0].title or "").strip()
            tname = title[2:].strip() if title.startswith("📋") else title
    except Exception: pass
    if not tname:
        try:
...
# 1) Thread.edit をデバウンス（同一スレッド連続 rename を 1 回に集約）
if not hasattr(_d.Thread, "_orig_edit"):
    ...
    async def _w(th:_d.Thread):
        await _a.sleep(0.25)                 # 連続呼び出しを同時刻にまとめる
...
# 2) “色替え系リスナー”を静かに無効化（on_interaction で rename だけをやる物を除外）
try:
...
# 3) handle_task_action の中で確実に 1 回だけ色替え（ステータスと完全連動）
def _em(st): return {"pending":"🟥","accepted":"🟨","completed":"🟩","abandoned":"⚠️","declined":"❌"}.get(st,"⚪")
...
async def _find_thread(inter, task_id, task_name):
    ...
    if tid:
        ...
        if isinstance(th,_d.Thread):
            return th
    # 2) 文脈（スレッド内 or 親メッセージ thread / 親chのthreads）
    msg=inter.message
...
async def handle_task_action(interaction, action, task_id, assignee_id, instructor_id, current_status):
    ...
    if new!=cur:
        await th.edit(name=new)          # ← ここが集約され、最終名だけ 1 回 PATCH
...
async def _find_thread(inter, task_id, task_name):
    ...
    # 2) 文脈から（スレッド内→channel、親→message.thread、親chのthreads）
    msg=inter.message
    th = msg.channel if isinstance(msg.channel,_d.Thread) else getattr(msg,"thread",None)
    if not isinstance(th,_d.Thread):
        try:
...
async def handle_task_action(interaction, action, task_id, assignee_id, instructor_id, current_status):
    ...
    if new!=cur:
        await th.edit(name=new)         # ← この1回だけで色を反映
        logger.info(f"[link] '{cur}' -> '{new}' (action={action}, id={task_id})")
    else:
        logger.info(f"[link] no change: '{cur}' (action={action})")
# ==== /LINK_HANDLE ====
...
# 直前の定義を退避し、最後にこれを使わせる
__orig_handle = globals().get('handle_task_action')

async def handle_task_action(interaction, action, task_id, assignee_id, instructor_id, current_status):
    # 1) まず元の処理を実行（DB更新・Embed切替など）
    if callable(__orig_handle):
        await __orig_handle(interaction, action, task_id, assignee_id, instructor_id, current_status)
    ...
    # 5) 先頭絵文字を 1 回だけ更新（集約パッチが入っていれば多重呼び出しでも1回にまとまる）
    cur=th.name or ""
    em=__em(target)
    for e in ("🟥","🟨","🟩","⚠️","❌","⚪"):
        if cur.startswith(e):
...
# 既存 on_ready をラップして、余分な on_interaction リスナーを無効化→正規リスナーを登録
...
async def on_ready():
    ...
    try:
        # 既存の on_interaction リスナーを全削除（このあと正規リスナーを 1 個だけ追加）
        if hasattr(bot, "_listeners") and "on_interaction" in bot._listeners:
    ...
    async def __canon_interaction(inter: _d.Interaction):
        try:
            ...
            # まず元の handle_task_action（直前に定義された最新版）を呼ぶ → DB/Embed/ボタン更新
            ...
            # 望ましい状態をアクションから直接決定（DB伝播待ちレースを回避）
            target = _tgt(action)
            if not target: return

            # 対象スレッドを特定（thread_idなければ補完）
            ...
            # 先頭絵文字を 1 回だけ更新（集約パッチが入っていれば自然と 1 回に統合）
            cur = th.name or ""
            em  = _em(target)
            for e in ("🟥","🟨","🟩","⚠️","❌","⚪"):
                if cur.startswith(e):
...
def _build_embed(title, due_dt, status):
    ...
    e=_d.Embed(title=title or "📋 タスク", description=f"**期日: {due_str}**", color=color)
    # Status表示（必要ならフィールドを足す）
...
async def on_ready():
    ...
    async def __canon2(inter:_d.Interaction):
        try:
            ...
            # 2) DBのステータスを更新（このハンドラが単独で完結）
            _db_exec("UPDATE tasks SET status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", (new_status, task_id))

            # 3) Embed/ボタンを更新
            title = (inter.message.embeds[0].title if inter.message.embeds else f"📋 {task_name}") or f"📋 {task_name}"
...
def __c2_build_embed(title, due_dt, status):
    ...
    e=__d.Embed(title=title, description=f"**期日: {due_str}**", color=color)
    e.add_field(name="Status", value=status, inline=True)
    return e
...
async def __canon2_install():
    ...
    try:
        # 既存の on_interaction を全て消して、1 本だけ登録
...
async def __c2_handler(inter: __d.Interaction):
    try:
        ...
        if new!=cur:
            await th.edit(name=new)    # Thread.edit は集約パッチで1回に統合される
...
# 既存リスナーは触らず、rename専用のハンドラを“前もって”登録
...
# すでに _orig_edit が無ければ退避
if not hasattr(__d.Thread, "_orig_edit"):
    __d.Thread._orig_edit = __d.Thread.edit

# 状態: tid -> {desired:str, task:Task|None, last:float}
__REN_STATE = {}
__COOLDOWN_SEC = 2.5  # この秒数以内の連続 rename は捨てる
...
_COOLDOWN = 4.0       # 同一スレッドの連続 rename はこの秒数以内なら捨てる
_DEBOUNCE  = 0.20     # 呼び出しをまとめるための待ち
...
def __em(st): return {"pending":"🟥","accepted":"🟨","completed":"🟩","abandoned":"⚠️","declined":"❌"}.get(st,"⚪")
...
async def __rename_only_handler(inter: __d.Interaction):
    try:
        ...
        new=new.lstrip()
        if new!=cur:
            await th.edit(name=new)            # ← Thread.edit は既に集約パッチで1回に統合
            logger.info(f"[c2] '{cur}' -> '{new}' (action={action}, id={task_id})")
        else:
            logger.info(f"[c2] no change: '{cur}' (action={action})")
    except Exception as e:
...
async def setup_hook():
    # 競合する on_interaction リスナーを消して、rename専用だけ登録
    try:
        if hasattr(bot,"_listeners"):
            bot._listeners["on_interaction"]=[]
    except Exception: 
...
# ==== ASSIGN_FALLBACK (robust @bot @user, 期日, タスク名) ====
import re as __re, sqlite3 as __sq, discord as __dd, datetime as __dt

@bot.event
async def setup_hook():
    @bot.listen("on_message")
    async def __assign_fallback(message: __dd.Message):
        try:
            ...
            # 対象ユーザー（Bot以外のメンション）
            assignees = [m for m in message.mentions if m.id != bot.user.id]
            if not assignees:
                await message.reply("❌ 指示対象のユーザーをメンションしてください。")
                return

            # 先頭のメンション（<@...>）を全部剥がして本文だけに
            text_wo_mentions = __re.sub(r'<@!?[0-9]+>', '', text).strip()

            # 最初の区切り（半角/全角カンマ）を見つける
            m = __re.search(r'[，,]', text_wo_mentions)
            if not m:
                await message.reply("❌ 形式: `@bot @ユーザー, 期日, タスク名`（**半角`,` を2つ**）")
                return

            rest = text_wo_mentions[m.end():].strip()
            # 期日, タスク名 に分割（全角/半角カンマ両対応）
                ...
                if ch:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # thread_id / message_id を保存（以後の色替えで使用）
                            try:
                                conn = __sq.connect('reminder_bot.db'); cur = conn.cursor()
            ...
            try:
                await message.reply("❌ 解析中にエラーが発生しました。")
            except Exception:
                pass
    logger.info("[assign] fallback on_message handler installed via setup_hook")
# ==== /ASSIGN_FALLBACK ====
