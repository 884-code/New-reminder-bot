...
def insert_task(gid:int, iid:int, aid:int, name:str, due:datetime, mid:int=None, cid:int=None) -> int:
    db_exec("INSERT INTO tasks (guild_id,instructor_id,assignee_id,task_name,due_date,message_id,channel_id) VALUES (?,?,?,?,?,?,?)", 
...
def build_embed(trow, status:Optional[str]=None) -> discord.Embed:
    st = status or trow[6]
    try:
        due_ts = int(datetime.fromisoformat(str(trow[5])).timestamp())
    except Exception:
        due_ts = int(datetime.strptime(str(trow[5]), "%Y-%m-%d %H:%M:%S").timestamp())
    ...
    emb.set_footer(text=f"Task ID: {trow[0]}")
    return emb
...
class _BaseBtn(discord.ui.Button):
    ...
    async def _handle(self, it:discord.Interaction, new_status:str):
        ...
        if not t:
            await it.response.send_message("❌ Task not found.", ephemeral=True)
            return
        t = t[0]
        if it.user.id != t[3]:
            await it.response.send_message("❌ You are not the assignee.", ephemeral=True)
            return
...
class AcceptButton(_BaseBtn):
    def __init__(self, tid:int):
        super().__init__("✅ Accept", discord.ButtonStyle.success, f"accept_task_{tid}")
    async def callback(self, it):
        await self._handle(it, 'accepted')

class DeclineButton(_BaseBtn):
    def __init__(self, tid:int):
        super().__init__("❌ Decline", discord.ButtonStyle.danger, f"decline_task_{tid}")
    async def callback(self, it):
        await self._handle(it, 'declined')

class CompleteButton(_BaseBtn):
    def __init__(self, tid:int):
        super().__init__("📝 Complete", discord.ButtonStyle.success, f"complete_task_{tid}")
    async def callback(self, it):
        await self._handle(it, 'completed')

class AbandonButton(_BaseBtn):
    def __init__(self, tid:int):
        super().__init__("⚠️ Problem", discord.ButtonStyle.danger, f"abandon_task_{tid}")
    async def callback(self, it):
        await self._handle(it, 'abandoned')

class UndoButton(_BaseBtn):
    def __init__(self, tid:int):
        super().__init__("↩️ Undo", discord.ButtonStyle.secondary, f"undo_completion_{tid}")
    async def callback(self, it):
        await self._handle(it, 'accepted')
...
async def test_cmd(ctx:commands.Context):
    ...
    task = get_task(tid)
    ch = await ensure_personal(ctx.guild, ctx.author)
    view = TaskView(tid, ctx.author.id, ctx.author.id, 'pending')
    if ch:
        await ch.send(ctx.author.mention, embed=build_embed(task), view=view)
    else:
        try:
            await ctx.author.send(embed=build_embed(task), view=view)
        except:
            pass
...
async def channels_slash(it:discord.Interaction):
    if not it.guild:
        await it.response.send_message("❌ サーバー内で実行してください", ephemeral=True)
        return
    if not is_admin(it.user.id, it.guild.id):
        await it.response.send_message("❌ Admin only", ephemeral=True)
        return
